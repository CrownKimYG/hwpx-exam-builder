...
1. 로컬 화면에 `.hwpx` 문제은행을 드래그 앤 드롭하거나 선택합니다.
2. 인식된 전체 문항 수와 객관식·단답식 수를 확인합니다.
3. 이전·다음 버튼으로 원본 문서 페이지를 확인합니다.
4. 문항 번호를 `4 1 5 6`처럼 띄어쓰기로 입력하고 Enter를 누릅니다.

문항은 입력한 번호 순서 그대로 배치됩니다. 사용자 템플릿을 넣지 않으면
`{{QUESTIONS}}` 연속 삽입 지점이 있는 내장 빈 2단 템플릿을 사용해 HWPX를
브라우저에서 생성하고 내려받습니다.

**미주 숨기기**를 선택하면 복사된 미주의 글자와 수식을 1pt·흰색으로
처리합니다. 템플릿의 `#해설` 문단은 해설 시작 위치로 인식하며, 최종
시험지에서는 같은 서식을 유지한 `해설` 제목으로 바뀝니다.

사용자 템플릿의 `#1`, `#2` 문제 슬롯과 `CLICK_HERE` 누름틀도 지원합니다.
중첩 표에 같은 번호의 슬롯이 반복되면 실제 바깥쪽 슬롯 하나만 사용하며,
생성 결과의 문항 미주·정답·해설 수와 리소스 연결을 검사한 뒤에만 다운로드합니다.
